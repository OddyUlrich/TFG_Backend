...
public class ExerciseTemplateDataDTO {
    ...
    public ExerciseTemplateDataDTO(Exercise exercise, List<ExerciseFile> files) {
        ExerciseDTO exerciseDTO = new ExerciseDTO(exercise.getId(), exercise.getName(), exercise.getStatement(), exercise.getExerciseBattery().getName(), exercise.getRequiredRules(), exercise.getForbiddenRules(), exercise.getTags().stream().map(Tag::name).toList());
